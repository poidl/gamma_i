...
if strcmp(tvar,'ct')
    ...
    pr0 = zeros(size(p)); t = theta_from_t(s,t,p,pr0); tvar = 'theta';
end

inds = find(isfinite(s)); n = length(inds);
...
inds1 = find(abs(N2)<=eps); N2(inds1) = 0;

%   interpolate back onto original grid

N2_h = N2(1); N2_l = N2(n-1);

N2(2:n-1) = interp1(p_mid,N2(1:n-1),p(2:n-1));

if n>3
  N2(1) = N2_h-(N2(3)-N2(2))/2; N2(n) = N2_l+(N2(n-1)-N2(n-2))/2;   %  fix for top and bottom
else
  N2(1) = N2(2); N2(3) = N2(2);
end
