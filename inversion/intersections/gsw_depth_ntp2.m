...
ncr = 0;
for k = 1:n
    [sigl,sigu] = gsw_sig_vals(SA0,CT0,p0,SA(k),CT(k),p(k));
    e(k) = sigu - sigl;
    if k > 1
        ...
        elseif e(k)*e(k-1) < 0            %  a crossing between k-1 and k bottles
            ...
            while success == 0
                iter = iter + 1;
                [SPc0,CTc0] = gsw_stp_interp([SA(k-1),SA(k)],[CT(k-1),CT(k)],[p(k-1),p(k)],pc0);
                [sigl,sigu] = gsw_sig_vals(SA0,CT0,p0,SAc0,Ctc0,pc0);
%                  pmid = 0.5*(p0 + pc0);
%                  sig1 = gsw_rho(SA0,CT0,pmid) - 1000;
%                  sig2 = gsw_rho(SAc0,CTc0,pmid) - 1000;
                ec0 = sigu - sigl;
                p1 = 0.5*(p(k-1) + pc0);
                ...
                if (pc1 < p(k-1)) | (pc1 > p(k))
                    ...
                    [SAns,CTns,pns,niter] = e_solve(SA(Idata),CT(Idata),p(Idata),e(Idata),k,SA0,CT0,p0);
                    if pns < p(k-1) | pns > p(k)
                        disp('****ERROR**** in gsw_depth_ntp')
                        return
                    else
                        success = 1;
                    end
