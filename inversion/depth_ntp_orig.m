function [sns,tns,pns] = depth_ntp_orig(s0,t0,p0,s,t,p)
...
ncr = 0;
for k = 1:n
    [sigl,sigu] = sig_vals_orig(s0,t0,p0,s(k),t(k),p(k));
    e(k) = sigu - sigl;
    if k>1
        ...
        elseif e(k)*e(k-1)<0           %  a crossing between k-1 and k bottles
            ...
            while success==0
                iter = iter + 1;
                [sc0,tc0] = stp_interp_orig([s(k-1),s(k)],[t(k-1),t(k)],[p(k-1),p(k)],pc0);
                [sigl,sigu] = sig_vals_orig(s0,t0,p0,sc0,tc0,pc0);
                ec0 = sigu - sigl;
                p1 = 0.5*(p(k-1) + pc0);
                ...
                if pc1<p(k-1) | pc1>p(k)
                    indsp = find(isfinite(s));
                    [sns,tns,pns,niter] = e_solve_orig(s(indsp),t(indsp),p(indsp),e(indsp),k,s0,t0,p0);
                    if pns<p(k-1) | pns>p(k)
                        disp('****ERROR**** in depth_ntp_orig')
                        return
                    else
                        success = 1;
                    end
                    ...
                    if abs(ec0)<=5.d-5 & eps<=5.d-3
                        ...
                        niter = iter;
                    elseif iter>10
                        [sns,tns,pns,niter] = e_solve_orig(s,t,p,e,k,s0,t0,p0);
                        success = 1;
                    else
                        pc_0 = pc0; 
                        ec_0 = ec0; 
