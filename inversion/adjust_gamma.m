...
inds = find(isfinite(gi+gn)); gi = gi(inds); gn = gn(inds);

%       on raw data

nn = length(gi); ng = 0; if ng~=0, bin_size = floor(nn/ng), end

gi_adj = nan*ones(ng,1);

[gi_srtd, k_srtd] = sort(gi); gn_srtd = gn(k_srtd);

gi_0 = gi_srtd(1); gi_1 = gi_srtd(nn);

gn_0 = gn_srtd(1); gn_1 = gn_srtd(nn);
...
%    [indsmgs,g(indsmgs),g(1:ngg,j0,i0),gi_adj]
    ...
    inds_g = find(isfinite(g)); ginds = nan*ones(size(g)); ginds(inds_g) = 1:length(inds_g);
    
%%  maximum and minimum casts

    gfac = 0.9;
    
    [gmax,indg] = nanmax(g(30,:)); ind_gmax = find(g(inds_g)==gmax);
...
%    [gmax,ind_gmax] = nanmax(g(inds_g)) 
    ...
    gg = g(:,j0,i0); indsgg = find(isfinite(gg)); ngg = length(indsgg)
    ...
    indsgg = find(gg>=(gmax-gfac*gmid)); ngg = length(indsgg)

    indsmgs = inds_g(ginds(1:ngg,j0,i0));
    
    gi_adj = g(indsmgs); gi_nodes = gi_adj;   
 ...
 %
    ...
    indsgg = find(gg<gmin+gfac*gmid); ngg = length(indsgg)   
...
figure, subplot(2,2,1)
            plot(gi,gn,'.'), grid on
            hold on, plot(gi_nodes,gi_adj,'c'), hold off
            
        subplot(2,2,2)
            plot(diff(gi_adj)), grid on
...
inds = find(abs(resids)<=2*sig); percent_of_data = 100*length(inds)/nn
            

gi = gi(inds); gn = gn(inds);

nn = length(gi); ng = 0; bin_size = floor(nn/ng)

gi_nodes = nan*ones(ng,1); gi_adj = nan*ones(ng,1);

[gi_srtd, k_srtd] = sort(gi);

indsmgs = nan*ones(ng,1);

for k = 1:ng
    inds = 1+(k-1)*bin_size : min(nn,k*bin_size);
    gi_nodes(k) = median(gi_srtd(inds));
    gi_adj(k) = median(gn(k_srtd(inds)));
    [dg_min,indss] = nanmin(abs(gn0(:)-gi_adj(k))); indsmgs(k) = indss(1);
end
...
inds0 = find(gn0==gn_0); inds0 = inds0(1);

inds1 = find(gn0==gn_1); inds1 = inds1(1);
