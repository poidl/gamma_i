function [sns,tns,pns,iter] = e_solve_orig(s,t,p,e,k,s0,t0,p0)
...
while success==0
    iter = iter + 1;
    pm = 0.5*(pl + pu);
    [sm, tm] = stp_interp_orig([s(k-1),s(k)],[t(k-1),t(k)],[p(k-1),p(k)],pm);
    [sigl, sigu] = sig_vals_orig(s0,t0,p0,sm,tm,pm);
    em = sigu - sigl;
    if el*em<0
        pu = pm;
        eu = em;
