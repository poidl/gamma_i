function [ave,percent2,epsilon_x,epsilon_y] = get_gradients(s,ct,p,g,ocean,n,longs,lats,handles)
...
s = change_ak(s,'==',nan,-99);
ct = change_ak(ct,'==',nan,-99);
p = change_ak(p,'==',nan,-99);
g = change_ak(g,'==',nan,-99);
ocean = change_ak(ocean,'==',nan,-99);
n = change_ak(n,'==',nan,-99);
%longs = longs(:);
%lats = lats(:);
%[longs2,lats2] = meshgrid(longs,lats);
...
if ntp_surfaces == 1
    ...
    %    end
    %    dj_toc
    ntpx = change_ak(ntpx,'==',-99,nan);
    ntpy = change_ak(ntpy,'==',-99,nan);
    ntpx = reshape(ntpx,nz,ny,nx);
    ntpy = reshape(ntpy,nz,ny,nx);
...
if ns_surfaces == 1
    ...
    %    end
    %    dj_toc
    nsx = change_ak(nsx,'==',-99,nan);
    nsy = change_ak(nsy,'==',-99,nan);
    nsx = reshape(nsx,nz,ny,nx);
    nsy = reshape(nsy,nz,ny,nx);
...
if sigp_surfaces == 1
    ...
    %    if handles.temp==1
    options(1) = 4;
    options(2) = handles.pr0;
    [sigpx,sigpy] = quick_gradients(s,t,p,g,ocean,n,longs,lats,options);
    %    elseif handles.temp==3
    %        [ntpx,ntpy] = ntp_gradients(s,ct,p,g,ocean,n,longs,lats);
    %    end
    %    dj_toc
    sigpx = change_ak(sigpx,'==',-99,nan);
    sigpy = change_ak(sigpy,'==',-99,nan);
    sigpx = reshape(sigpx,nz,ny,nx);
    sigpy = reshape(sigpy,nz,ny,nx);
    inds = find(isfinite(sigpx + sigpy));
    n_sigp = length(inds)
...
indsss = find(isfinite(g+s+nsx+ntpx+nsy+ntpy));
nn = length(indsss);
...
if nn>0
    ...
    sx_diff = ntpx(indsss) - nsx(indsss);
    sy_diff = ntpy(indsss) - nsy(indsss);
    ...
    nD = sum(D_bins);
    D_integral = dj_quad(x_bins,D_bins);
    D_bins = D_bins/D_integral;
    
    percent = 100*dj_quadi(x_bins,D_bins,-5,0);
    
    ave = dj_quad(x_bins,x_bins.*D_bins);
    N_stats = [ave,percent];
    
    logD_srtd = sort(logD0);
    nD = length(logD_srtd);            %   95th percentile
    ...
    if by_volume == 1
        indss5 = find(logD>=-5);
        percent1 = 100*length(indss5)/length(indsss);
        i3 = zeros(size(dp));
        i3_5 = i3;
        i3(indsss) = 1;
        i3_5(indss5) = 1;
        zz = i3.*cos_mat.*dp;
        zz5 = i3_5.*cos_mat.*dp;
        percent2 = 100*nansum(zz5(:))/nansum(zz(:));
        N_stats(3) = percent2;
    end
    ...
    [g_bins,x_bins] = hist(gg,100);
    g_bins = g_bins/sum(g_bins(:));
    ...
    if sum(size(gn))==0
        ...
        xmin = 0.99*min(gg);
        xmax = 1.01*max(gg);
        gmax = 1.01*max(g_bins);
        set(gca,'xlim',[xmin,xmax],'ylim',[0,gmax])
    else
        gnn = gn(inds_g);
        plot(gg,gg-gnn,'.')
        grid on
        if sum(size(limit_panel2))~=0
            set(gca,'xlim',[27,29],'ylim',[-0.2,0.2])
        end
    end
    ...
    title(['\gamma     ', num2str(gmean,4),'     ',  num2str(gstd,3)]);
...
%-------------------------------------------------------------------------- 
    ...
    minz = nanmin(logD2(:));
    [maxz0,indsm0] = nanmax(logD2(:));
    minz = max(round(minz),-9);
    maxz = round(maxz0);
    ...
    subplot(2,2,3)
    % whos, dj_pause(0)
    dj_pltmp(nanmean(longs),nanmean(lats'),logD2)
    grid on
    title([num2str(ave,3), '    ', num2str(p50,3), '    ', num2str(p95,3)])
    ...
    pmean = nanmean(p(:,:)');
    k0 = find(pmean>200);
    k1 = change(s,'>=',0,1); k1mean = nansum(k1(k0,:)');
    ...
    if perc5>=10
        title([num2str(perc5,4),'%']) %, '    ', num2str(p95,3)])
    else
        title([num2str(perc5,3),'%']) %, '    ', num2str(p95,3)])
    end
    ...
    inds = find(isfinite(g));
    ng = length(inds)
    inds = find(isfinite(s));
    ns = length(inds)
    inds = find(isfinite(ntpx));
    nntpx = length(inds)
    inds = find(isfinite(nsx)); 
    nnsx = length(inds)
    inds = find(isfinite(ntpy));
    nntpy = length(inds)
    inds = find(isfinite(nsy)); 
    nnsy = length(inds)
